...
const namesToDelete = [
  "Ascension Health & Vitality Crystal Bracelet",
  "Ascension Protection & Grounding Crystal Bracelet",
  "Ascension Abundance & Wealth Crystal Bracelet",
  "Ascension Cleansing & Higher Vibes Crystal Bracelet"
];

const finalCrystalsAndProducts = [
  {
...
Cleanse with sacred sage smoke, lavender water, or under the starry night sky. Recharge in moonbeams or alongside Selenite.`
  },
  {
    ...
    pricing: 699,
    stock: 50,
    images: ["/uploads/tiger_eye_crystal_bracelet.png"],
...
async function cleanAndSync() {
  try {
    const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/ascension';
    console.log('Connecting to MongoDB...');
    await mongoose.connect(mongoUri);
    console.log('Connected to MongoDB.');

    // 1. Delete duplicate old crystal products
    for (const name of namesToDelete) {
      const res = await Product.deleteMany({ name });
      console.log(`Deleted duplicates for "${name}": ${res.deletedCount}`);
    }

    // 2. Upsert final products
    for (const p of finalCrystalsAndProducts) {
      ...
      if (existing) {
        ...
        await existing.save();
        console.log(`Updated product: ${p.name} (₹${p.pricing}) with image ${p.images[0]}`);
      } else {
    ...
    // 3. Clean extracted_products.json
    ...
    // Filter out namesToDelete
    fileProducts = fileProducts.filter(fp => !namesToDelete.includes(fp.name));

    // Update or append final products
    for (const p of finalCrystalsAndProducts) {
    ...
    console.log('Successfully updated extracted_products.json without duplicates.');

    console.log('Verification: Listing all crystal products in DB:');
    const allCrystals = await Product.find({ category: 'Crystals' });
    allCrystals.forEach(c => console.log(` - ${c.name} | ₹${c.pricing} | ${c.images[0]}`));

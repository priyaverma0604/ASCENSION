    ...
    <>
      ...
      <div className={`glass max-w-7xl mx-auto rounded-2xl flex items-center justify-between transition-all duration-300 ${isScrolled ? 'shadow-md py-2 px-6' : 'shadow-sm py-3 px-6'}`}>
        ...
        <Link to="/" className="flex items-center group">
          <img src={`${logo}?v=3`} alt="Ascension by Sonali Bhasin Kumar" className="h-9 md:h-10 w-auto object-contain transition-all duration-300" />
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-7">
          ...
          >
            ...
            {/* Dropdown Container */}
            <div 
              className={`absolute left-1/2 -translate-x-1/2 mt-2 w-44 bg-white/95 backdrop-blur-md rounded-2xl shadow-xl border border-cream-dark/45 py-2.5 z-50 transition-all duration-300 origin-top transform ${
                sevaDropdownOpen ? 'opacity-100 scale-100 translate-y-0 visible' : 'opacity-0 scale-95 -translate-y-2 invisible pointer-events-none'
              }`}
            >
              <Link
                to="/ngo"
                onClick={() => setSevaDropdownOpen(false)}
                className={`block px-5 py-2.5 text-[10px] font-bold uppercase tracking-wider hover:bg-cream-light/60 hover:text-gold-dark transition-colors duration-200 ${
                  location.pathname === '/ngo' ? 'text-gold-dark bg-cream-light/35' : 'text-charcoal/85'
                }`}
              >
                Seva NGO
              </Link>
              <Link
                ...
                onClick={() => setSevaDropdownOpen(false)}
                className={`block px-5 py-2.5 text-[10px] font-bold uppercase tracking-wider hover:bg-cream-light/60 hover:text-gold-dark transition-colors duration-200 ${
                  location.pathname === '/community' ? 'text-gold-dark bg-cream-light/35' : 'text-charcoal/85'
                }`}
              >
                Community
              </Link>
              <Link
                ...
                onClick={() => setSevaDropdownOpen(false)}
                className={`block px-5 py-2.5 text-[10px] font-bold uppercase tracking-wider hover:bg-cream-light/60 hover:text-gold-dark transition-colors duration-200 ${
                  location.pathname === '/donate' ? 'text-gold-dark bg-cream-light/35' : 'text-charcoal/85'
                }`}
              >
                Donate
              </Link>
        ...
        <div className="lg:hidden mt-2 px-2 z-50 relative">
          <div className="glass rounded-2xl shadow-lg p-5 flex flex-col gap-2">
            ...
            <div className="flex flex-col border-b border-cream/50">
              ...
              <div 
                className={`flex flex-col pl-4 gap-2 transition-all duration-300 overflow-hidden ${
                  mobileSevaOpen ? 'max-h-40 pb-3 pt-1' : 'max-h-0'
                }`}
              >
                <Link 
                  to="/ngo" 
